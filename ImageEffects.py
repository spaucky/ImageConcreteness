...
def auto_cropper(image_file):
    ...
    if len(results[0].boxes.conf) == 0:
        ...
        cropped_img.save(os.path.join('CroppedOASIS','Cropped ' + image_file.rstrip('.jpg').strip() + '.jpg'))
...
def sabattier_effect(image_file):
    ...
    sabbatier_img.save(os.path.join('SabattierOASIS','Sabattier ' + image_file.rstrip('.jpg').strip() + '.jpg'))
...
def blur_effect(image_file,intensity,file_to_save):
    ...
    blurred_img.save(os.path.join('BlurredOASIS' + str(file_to_save),'Blurred ' + image_file.rstrip('.jpg').strip() + '.jpg'))
...
def pixelate_effect(image_file):
    ...
    pixelated_img.save(os.path.join('PixelatedOASIS','Pixelated ' + image_file.rstrip('.jpg').strip() + '.jpg'))
...
def sharpen_effect(image_file):
    ...
    sharpened_img.save(os.path.join('SharpenedOASIS','Sharpened ' + image_file.rstrip('.jpg').strip() + '.jpg'))
